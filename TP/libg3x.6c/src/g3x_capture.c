...
extern void  g3x_SetMaxImage(int m) { _MAXIMAGES_=m;      }
extern int   g3x_GetMaxImage(void ) { return _MAXIMAGES_; }
extern void  g3x_SetFrameNum(int frame) { framenum=frame;  }
extern int   g3x_GetFrameNum(void     ) { return framenum; }
extern void  g3x_SetBitRate(int bit_rate) { bitrate=bit_rate; }
extern int   g3x_GetBitRate(void        ) { return bitrate;   }
extern void  g3x_SetFrameRate(int frame_per_sec) { fps=frame_per_sec; }
extern int   g3x_GetFrameRate(void             ) { return fps;        }
extern void  g3x_SetPid(int force_pid) { pid=force_pid; }
extern int   g3x_GetPid(void         ) { return pid;    }
...
extern void g3x_UnplugCapture(void)
{ 
	if (!_PLUGGED_) return;
  fflush(framefile);
  fclose(framefile);
...
extern bool g3x_PlugCapture(const char *basename, int dlx, int dly, int urx, int ury)
{
  ...
  { fprintf(stderr,"\e[1;31m<g3x_PlugCapture> : Erreur Allocation Pixmap\e[0m\n"); return false;}  
  if (!(framefile=fopen(tempsnapname,"w")))
  { fprintf(stderr,"\e[1;31m<g3x_PlugCapture> : Erreur Création fichier temporaire <%s>\e[0m\n",tempsnapname); return false;}  
  return true;
}

extern bool g3x_Snapshot(const char *format, const char *basename, int w, int h)
{
  static uint   view=0;
...
extern bool g3x_FilmFrame(void)
{
...
extern bool g3x_MakeAvi(void)
{
	snprintf(command,2047,"mencoder -quiet mf://%s/*.jpg -mf fps=%d:type=jpg -oac copy -o %s.avi \
-ovc x264 -x264encopts bitrate=%d:subq=6:partitions=all:me=umh:frameref=5:bframes=3:b_pyramid:weight_b:threads=auto:pass=1"
  ...
  _r_=system(command);
	snprintf(command,2047,"mencoder -quiet mf://%s/*.jpg -mf fps=%d:type=jpg -oac copy -o %s.x264.avi \
...
extern bool g3x_MakeMpeg4(void)
{
	snprintf(command,2047,"mencoder -quiet mf://%s/*.jpg -mf fps=%d:type=jpg -o %s.mp4 -ovc lavc -lavcopts vcodec=mpeg4:vbitrate=%d:mbd=2:o=mpv_flags=+mv0:trell:v4mv:o=mpv_flags=+cbp_rd:last_pred=3:predia=2:dia=2:vmax_b_frames=2:vb_strategy=1:precmp=2:cmp=2:subcmp=2:preme=2:o=quantizer_noise_shaping=2:threads=12"
		       	           ,tempdirname,fps,capturename,bitrate);
  _r_=system(command);
	return true;
}

extern bool g3x_MakeFlv(void)
{
	snprintf(command,2047,"mencoder mf://%s/*.jpg\
	                       -mf fps=%d:type=jpg\
												 -of lavf -ovc lavc -nosound\
												 -lavcopts vcodec=flv:vbitrate=%d:mbd=2:o=mpv_flags=+mv0:trell:v4mv:o=mpv_flags=+cbp_rd:last_pred=3\
												 -o %s.flv"
